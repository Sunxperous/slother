var timetable = (function() {
  ...
  var popup = (function() {
    ...
    function internalClose(event) {
      if (_popup.status === _popup.statusTypes.HIDDEN) { return; }
      _popup.close();
    };
    function externalClose(event) {
      if (_popup.status !== _popup.statusTypes.READ_ONLY) { return; }
      if ($(event.target).attr('id') !== 'popup_wrapper') { return; }
      _popup.close();
    }
    function displayDetailsOnClick(event) {
      if (_popup.status !== _popup.statusTypes.HIDDEN) { return; }
      _popup.status = _popup.statusTypes.READ_ONLY;
    ...
    _popup.status = _popup.statusTypes.HIDDEN;
    _popup.displayDetailsOnClick = displayDetailsOnClick;
    ...
    $('#event_details :input').change(function(event) { // Disable outside close popup on edit.
      _popup.status = _popup.statusTypes.EDITED;
    });
